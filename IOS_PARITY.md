...
| **Settings** | | | | | |
| Settings | `/settings` | `/settings` | ✅ | ✅ | **Apple HIG rebuild + Account/Security/Permissions sections 2026-05-14**. Profile card → `/edit-profile`. New Account section: Edit profile · Security (→`/security`) · Permissions (→`/permissions`). Plus existing: Subscription, Appearance, Notifications, Privacy, Support, About, Sign out, Delete account. Primitives extracted to `components/SettingsPrimitives.tsx` (shared with /security). |
