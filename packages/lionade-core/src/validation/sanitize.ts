 ...
 *
 * Pure functions. No I/O. Imported by both web and iOS.
 * Moved from web /lib/sanitize.ts on 2026-05-13.
 *
 * Note: pattern detection is intentionally conservative — false positives are
 * preferable to false negatives. If a real signup gets blocked, ask the user
 * to use different text. The threat model is: a malicious actor pasting
 * unsanitized content into our DB or rendering surface.
 */
...
export function sanitizeLoginForm(raw: {
  email: string;
  password: string;
}): SanitizeResult<{ email: string; password: string }> {
  const data = {
    email:    sanitizeEmail(raw.email),
    password: sanitizePassword(raw.password),
  };
