...
export default function LoginPage() {
  ...
  // OAuth signup must assert 13+ like email signup (which collects a DOB). This
  // checkbox gates the Google/Apple buttons on the signup tab so an OAuth user
  // cannot create an account without the same affirmation (COPPA).
  ...
  const handleGoogleAuth = async () => {
    // Signup must affirm 13+ like email signup. Returning users on the login tab
    // already have accounts, so they are not gated.
                ...
                <div>
                  ...
                  {step === 1 && (
                    <div className="space-y-4 animate-slide-up">
                      {/* 13+ age gate, mirrors the DOB requirement on email signup.
                          The OAuth buttons stay disabled until it is checked. */}
                      <label className="flex items-start gap-2.5 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={ageConfirmed}
                          onChange={(e) => setAgeConfirmed(e.target.checked)}
                          className="mt-0.5 h-4 w-4 rounded border-white/20 bg-white/5 accent-gold"
                        />
                        <span className="text-cream/70 text-xs leading-snug">I confirm I am at least 13 years old.</span>
                      </label>
                      {/* Google OAuth */}
                      <button
                        type="button"
                        ...
                        className="w-full flex items-center justify-center gap-3 py-3.5 rounded-xl font-semibold text-sm transition-all duration-200 bg-white text-gray-700 hover:bg-gray-100 active:scale-[0.98] shadow-sm disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-white"
                      >
                        <svg width="18" height="18" viewBox="0 0 48 48"><path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"/><path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"/><path fill="#FBBC05" d="M10.53 28.59a14.5 14.5 0 0 1 0-9.18l-7.98-6.19a24.0 24.0 0 0 0 0 21.56l7.98-6.19z"/><path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/></svg>
                        Continue with Google
                      </button>
